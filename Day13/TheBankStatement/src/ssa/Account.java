...
public class Account
{
    ...
    public double deposit(double depositAmt) {
    	if (depositAmt > 0) {
    		...
    		System.out.printf("Error: invalid transaction. Reason: Invalid deposit amount.");
    	}
    	return getBalance();
    }
    ...
    public void transferFrom(Account remFromAcct, double transferAmt) {
    	...
    	if ((number2 >= number1) && (number1 > 0)) {
    		...
    		if (number1 <= 0) {
    			System.out.println("Error: Transfer aborted. Reason: Invalid transfer amount.");	
    		} else {
    			System.out.println("Error: Transfer aborted. Reason: Insufficient Funds.");
    		}
    	}
    }
    ...
    public String print() {
    	String stringId = String.format("%1$3s", Integer.toString(this.getId()));
    	String stringDesc = String.format("%1$-28s", this.description);
    	String stringBalance = String.format("%1$13s", nf.format(this.getBalance()));
    	return stringId + " " + stringDesc + " " + stringBalance;
    }
